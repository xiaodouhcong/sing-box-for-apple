...
public class CrashReportManager: ObservableObject {
    ...
    private nonisolated static let readReportsKey = "readCrashReports"
    ...
    private nonisolated static func archivePendingCrashLogs() {
        ...
        private nonisolated static func collectAndArchiveCrashArtifactsViaHelper() {
            ...
            for crashLog in artifacts.crashLogs {
                if crashLog.fileName == CrashReportArchive.configFileName {
    ...
    private nonisolated static func readAndCleanConfigSnapshot() -> String? {
        let url = FilePath.workingDirectory.appendingPathComponent(CrashReportArchive.configFileName)
        guard let content = try? String(contentsOf: url, encoding: .utf8),
        ...
        return files
            ...
            .compactMap { url -> CrashReport? in
                ...
                return CrashReport(
                    ...
                    date: date,
                    fileURL: url,
                    isRead: readSet.contains(url.lastPathComponent),
                    origin: origin
                )
            }
    ...
    public nonisolated func loadContent(for report: CrashReport) async -> String {
        await BlockingIO.run {
            let contents = CrashReportArchive.readContents(for: report.fileURL)
            let content = CrashReportArchive.displayContent(for: contents)
            if content.isEmpty { return content }

            if let metadata = CrashReportArchive.readMetadata(for: report.fileURL) {
                let metadataText = CrashReportArchive.formatMetadata(
                    CrashReportMetadataBuilder.normalized(
                        metadata,
                        content: content
                    )
                )
    ...
    public nonisolated func loadFileContent(_ file: CrashReportFile) async -> String {
        await BlockingIO.run {
    ...
    public func markAsRead(_ report: CrashReport) {
        var readSet = Self.readReportsSet()
        readSet.insert(report.id)
        UserDefaults.standard.set(Array(readSet), forKey: Self.readReportsKey)
    ...
    public nonisolated func delete(_ report: CrashReport) async {
        ...
        await MainActor.run {
            var readSet = Self.readReportsSet()
            readSet.remove(report.id)
            UserDefaults.standard.set(Array(readSet), forKey: Self.readReportsKey)
            reports.removeAll { $0.id == report.id }
            unreadCount = reports.filter { !$0.isRead }.count
        }
    }
    ...
    public nonisolated func deleteAll() async {
        ...
        await MainActor.run {
            UserDefaults.standard.removeObject(forKey: Self.readReportsKey)
            reports.removeAll()
            unreadCount = 0
        }
    }

    private nonisolated static func readReportsSet() -> Set<String> {
        Set(UserDefaults.standard.stringArray(forKey: readReportsKey) ?? [])
    }
